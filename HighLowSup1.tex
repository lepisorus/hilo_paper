...
%\plr{huh, nothing: since we assume number of ears is Poisson and the chance of replanting each ear is independent; the number of replanted ears is Poisson with a mean that is determined by the number of ears needed to replant}
...
%\plr{we could just cut the remining discussion.}
...
{\fontsize{8}{11}\sf
   ...
   S. America & Highland adaptation & 164 & 230 (71.3\%) &  $P<10^{-5}$  \\
    ...
    \hline\hline
    \multicolumn{5}{l}{\st{$^a$ Departure from a null expectation that PHS \textit{P}-values as an index of haplotype length is not associated}}\\
     \multicolumn{5}{l}{\st{with SNPs involved in highland adaptation.  The test was performed by Fisher's exact test.}}\\
    \end{tabular}
...
\begin{figure}[h]
  \begin{center}
    ...
    \caption{Pattern of decay of linkage equilibrium in Mesoamerica (A) and S. America (B).  Red and blue dots represent low- and highland population, respectively.  $r^2$ values were calculated as a statistics and averaged within 10-bp bins of distance between SNPs.  The \emph{x}- and \emph{y}-axes represent distance between SNPs (kb) and average $r^2$ values.}
    \label{supp:LD}
  \end{center}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Finally, we tested whether PHS test supports highland and lowland adaptation scenario (summarized in Table~S3).
...
Thus, we compared the empirical \st{quantile} of PHS test as an indicator of haplotype length given allele frequency (Pr($PHS_{xA}\leq PHS_{null|p}$) in Materials and Methods).
We just say that the PHS test is consistent if the empirical quantile in highlands is smaller than that in lowlands (haplotype length is longer as the empirical quantile is smaller).
\st{If we extract random SNPs, the probability that the haplotype length in highlands is longer than that in lowlands is expected to be 50\% by chance.
However, we rejected the null hypothesis by a Fisher's exact test in all cases (Table~S3).}
