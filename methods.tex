...
where $F_{ST\_O}$ and $F_{ST\_E}$ are observed and expected $F_{ST}$ values and $p\pm 0.05$ is the set of loci in the same 0.05 bin of mean allele frequency of the SNP across both highland and lowland populations. \jri{Sho is this correct or is the probability comparing all loci within 0.05 frequency of the given SNP?  That is, if my SNP is at MAF 0.07, does the probability only include things between 0.05-0.1 or is it going from 0.02-0.12?}
