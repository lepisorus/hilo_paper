...
\begin{figure*}[tb]   
  \begin{center}
   ...
   \vspace{-3mm}
   \caption{
   (A) Sampling locations of landraces.  Red, blue, yellow and light blue dots represent Mexican lowland, Mexican highland, S. American lowland and S. American highland populations, respectively.  
   \plr{Looks like some of those ``Mexican'' dots are in Guatemala? I can't think of a better name than `Mexican high/lowland' though.}
...
\comst{Peter- we used Model IA for both Mexican and S. American populations, so I'd like to keep it (I have no idea to improve these sentences, though...)}
In total, we included 18,745 synonymous and noncoding SNPs for the Mexican populations in Models IA and IB, 14,508 for the S. American populations in Model I\st{A} and 11,305 for the Mexican lowland population and the S. American populations in Model II.  
...
\plr{PLoS has a new policy agains saying ``data not shown''; throw results into a supplement or strike that phrase?}
\comst{We'll go to Genetics. I didn't find this policy in Author Instruction of Genetics.}
...
\st{We did not use ``full'' model that incorporates all the four populations because parameter estimation under this model is computationally infeasible.}
...
\paragraph{Model IA}
This model is applied separately to both the Mexican and the S. American populations.
...
\plr{grrr, ``data not shown''}\comst{haw!!}
...
\plr{Perhaps a brief sentence saying didn't do a four population model because inference becomes slow and unreliable with so many parameters.}
\comst{When I talked to Ryan, he said, "it actually works", but, yeah, I agree and added a sentence.}
...
$\bar{d_{ij}}$ is the genome-wide mean of distances over which individuals \plr{$i$ and $j$} are identical, 
...
\plr{This is (one minus) the quantile of the PHS value.}
...
\plr{again with the ``data not shown'' -- the code for this can be provided? or posted to datadryad?}
...
If an allele is beneficial at high elevation and fixed in the Mexican highlands but is deleterious at low elevations, then at equilibrium it will be present at low frequency at migration-selection balance in nearby lowland populations \citep{haldane1948theory,slatkin1973geneflow}.
...
\jri{Peter are the braces in the right place?}
\plr{yes: see code below; I've interpreted the expression a bit.}
