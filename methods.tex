...
We included one individual from each of 94 open-pollinated landrace maize accessions from high and low elevation sites in Mexico and S. America (Table~S1).  Accessions were provided by the USDA germplasm repository or kindly donated by Major Goodman (North Carolina State University).  
...
%please fill in X
GBS data were obtained for a subset of 87 of the landrace accessions (Table~S1). 
...
Despite an extremely high heterozygote error rate, our GBS data should be informative given the high correlation in allele frequencies between data sets ($r=0.89$; Figure~SX \st{change after finishing intro and results}) and a lack of major allele or reference bias (data not shown).
...
The \emph{P}-value for an allele $A$ with frequency $p$ at position $x$ was calculated such that Pr($PHS_{xA}\leq PHS_{null|p}$), where $PHS_{null|p}$ are the PHS values for all alleles with frequency $p$ across the genome. 
\plr{Do you mean that the $P$-value for allele $x$ is the proportion of alleles of the same frequency genome-wide that have a larger PHS value? (and, should be PHS, not PHA?)}
\st{You are right. PHA is a typo.  Corrected.}

Genetic distances were obtained from the IBM population \cite[]{Ganal_2011_22174790}.  
...
%MBH:  May need to think about rerunning this given the errors in Gerke's genetic map. 
...
%See above, Shohei.  The citation is Mercer, K, A. Mart’nez-V‡squez, H. R. Perales. 2008. Asymmetrical local adaptation of maize landraces along an altitudinal gradient. Evolutionary Applications 1: 489-500
% Done!! 
