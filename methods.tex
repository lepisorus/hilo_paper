...
\rev{We identified outlier PHS values using an empirical quantile, calculated as Pr($PHS_{xA}\leq PHS_{null|p}$), where $PHS_{null|p}$ are PHS values for alleles at frequency, $p$.  The smaller the statistic is, the longer the length of a haplotype around the allele is}.
