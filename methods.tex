...
(The fitness of homozygous is assumed to not affect the probability of establishment.)
...
Each of these numbers of offspring has a mean that depends on whether the field is replanted with new stock, and whether ears are chosen from this field to replant other fields, so the total number of offspring is actually a mixture of Poissons; these means, and more details of the computations, are found in the Appendix. % \ref{apx:demographic_model}.}
At these parameter values, the variance in number of offspring, $\xi^2$, is between 20 (for wild-type) and 30 (for $s_b=0.1$), and the dispersal distance (mean distance between parent and offspring) is $\sigma=3.5$km.\\
...
\plr{Move section ``Neutral alleles\ldots'' from the Appendix up to here?}

% Modeling of highland alleles which are neutral in the lowlands is more difficult, but approximations are possible (see \st{Appendix or Supporting Information?}).
