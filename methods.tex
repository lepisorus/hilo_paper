...
To identify outlying PHS values, we used the empirical \st{quantile} \comst{I think the reviewers 2 does not like this term}, calculated as the proportion of alleles of the same frequency genome-wide that have a larger PHS value; \st{we calculated Pr($PHS_{xA}\leq PHS_{null|p}$), where $PHS_{null|p}$ is PHS values for alleles in frequency, $p$}.
...
(b) by moving from highland population to the other my migration;
and 
...
we first evalute the rate at which we expect an allele that provides a selective advantage at higher elevation to arise by new mutation in a highland region ($\mutrate$), 
...
\plrnote{JRI:
    Evidence that long-distance migration is rare and probably not very important.
}
...
\plr{
(Note that in a panmictic population, $\xi^2 \approx N/N_e$.)
}
...
(by expanding the generating function near 1, see \citep{jagers1975branching}; or \citet{lambert2006probability} for more sophisticated models).
}
...
The calculation above does not take this into account,
but exact numerical calculation of the chance of fixation of a mutation 
as a function of the location 
where it first appears indicates that the approximation is quite good (see Figure~\ref{sfig:prob_estab}); 
