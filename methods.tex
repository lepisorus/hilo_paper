...
\jri{if we go genetics: leave it in; if we go plos genetics, i vote for removing sentence entirely rather than putting everything in supp. though that would ideally be the better solution}
...
%\plr{``data not shown''}
\jri{ again, keep if Genetics, remove sentence or remove ``data not shown'' if we go to PLoS Gen.}
...
\plr{again with the ``data not shown'' -- the code for this can be provided? or posted to datadryad?} \jri{sho: can we just post mathematica notebook (or perl or whatever you used) to figshare?}
