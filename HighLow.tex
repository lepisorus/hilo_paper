...
\newcommand{\plr}[1]{\textcolor{peterpurple}{ \emph{(#1)}} }
...
We evaluated these rates using theory validated by simulation.
...
but will require significant thinking (which I'd like to do\dots).
}
...
The latter probability that a single new mutant allele providing benefit $s$ to heterozygotes at high elevation
will increase in frequency and fix in the high elevation patch
is approximately $2s$ divided by the variance in offspring number \citep{jaegers}.
...
reducing the chance of fixation of an allele arising there.
This scenario has been well-studied in theoretical models by \citet{pollack} and \citet{barton},
but it is not immediately obvious how the demographic details of maize 
(an altitudinal gradient; high variance in number of offspring; both discrete and continuous population structure)
affects the analyses in this work.
As described in Appendix XX, 
we built on the demographic model used in \citet{joost} to numerically calculate the chance of fixation of a beneficial allele
as a function of the location it first appears,
but found that the simple approximation is quite good.

\plr{could alternatively: (a) describe demographic model in more detail; (b) describe the math; or (c) cut that down further.  
Should probably describe demographic model at least a bit.}
...
Denoting this probability by $\mutrate$,
\begin{align}
  \mutrate = \frac{2 \mu \rho A s_b$}{\xi^2} .
\end{align}

The corresponding chance that an allele moves from one highland population to another is less intuitive,
and the subject of \citep{ralphcoop2013}.
...
maintained at migration-selection balance \citep{slatkin}.
...
being present at distance $R$ from the highlands at frequency $C \exp(- R \sqrt{2s_m} / \sigma)$,
...
Multiplying by the population density gets the predicted number of individuals,
...
$N/2  \exp(- R \sqrt{2s_m} / \sigma)$ is equal to the probability that there are any highland alleles present at distance $R$
multiplied by the mean number of these, given that there are some present.
...
This allows us to put an upper bound on the rate of migration of high altitude-adapted alleles between two highlands.
(Such migrant alleles still need to fix; however, 
since such alleles arrive by migration, which are planted out into a field,
it is not too bad, and conservative, to neglect the factor of $2s_b/\xi^2$,
if we use the number of ears used to plant the region for $N$.)
...
\plr{Doesn't ``rapid decay'' make you less worried about hitchhiking?}
...
we get that with $10^{-1} \le s_b \le 10^{-4}$,
a single-base mutation with mutation rate $\mu=10^{-8}$ would still take at least 10,000 generations to appear and fix.
A kilobase-sized target with mutation rate of $\mu=10^{-5}$
with strong selection $s=.1$ would fix in only 10 generations,
while weakly selected alleles would take hundreds or thousands of generations.
(Note that at these values $\Tmut = 1/\mutrate = \mu s_b \times 10^5$.)
\plr{describe this better.}
...
\plr{uh-oh, better describe it more up there}
we have estimated that $\sigma \approx 2$ kilometers per generation,
so with $10^{-1} \ge s_m \ge 10^{-4}$ the distance $\sigma/\sqrt{2s_m}$ is between 4 and 150 kilometers.
\plr{put bounds on $\sigma$}
The area of the Andean highlands is about 500 km$^2$ \plr{is this really it?}
which we estimate was planted from around $N=14,000$ plants each year \plr{add more details}.
...
In other words, it is unlikely that an allele that is deleterious at low elevation with $s_m=10^{-3}$ 
would ever transit from the Mexican to the Andean highlands.
If $s_m=10^{-4}$ it is still expected to take $\Tmig = 1.8 \times 10^8$ generations.
Shorter distances could be transited by very weakly deleterious alleles --
