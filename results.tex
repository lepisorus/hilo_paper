...
-- if the distance between highland patches $R$ is 2,000 km (or if $\sigma$ is twice as large)
...
Finally, the rough calculations with coalescent theory above  \plr{or, below?}
show that even neutral alleles are not expected to transit between the Mexican and Andean highlands
within 4,000 generations if they were not already present.
Since the calculation of $\Tmig$ does not include the time for mutation-selection balance to be reached,
this lower bound also applies to weakly deleterious alleles,
suggesting that any form of shared adaptation is unlikely if it does not arise from shared standing variation.
