...
we also confirmed that the change of the strength of the bottleneck had almost no influence on the null distributions of $F_{ST}$ values (not shown).  In other words, our conclusion of rare convergent adaptation at the molecular level is robust against the strength of the domestication bottleneck.}
...
From the theoretical arguments above,
it seems that adaptation to high elevation
probably occured either  
through a combination of 
selection on standing variation,
and independent \textit{de novo} mutation.
The fact that we do not see evidence in the data of convergent responses to selection
suggests that selection acted on different loci in the two regions;
since the selected phenotypes are similar \plrnote{could we say ``the same''?},
either would require a fairly large target for selection, spread across many genes.
This agrees with our intuition that the traits under selection could easily be highly polygenic.
In fact, if the traits were not sufficiently polygenic, 
the calculations above suggest that adaptation by \textit{de novo} mutation would not have occurred at all.
As for standing variation,
since cultivated maize has retained high levels of diversity,
...
leading to a significant role of chance in which alleles are selected on,
as well the strength of the signal of $F_{ST}$.
This is particularly true for the rare end of the frequency spectrum, 
where we expect most weakly deleterious standing variation.
Epistasis could make it even less likely that the same allele is shared between regions.
}

\plrnote{``crossing experiments between high-altitude populations would be most informative here'' --nick}
