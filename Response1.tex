...
\response{
    We certainly agree that the demography should just come down to these parameters;
    we decided to explicitly work through the full model to avoid making mistakes in the process of handwaving
    ...
    Call us conservative, but we like having a nice, concrete model to feel solid about.
...
\response{
    ...
    To make this more clear, we've separated out the geographic description into a paragraph ``Geographic distribution''.
}
...
\response{
    We realized that since the coalescent theory calculations come to the same conclusion, and are more general,
    that the section involving migration-selection balance wasn't essential to the paper,
...
\response{
    Whoops!  We've taken the easy way out, redefining $\xi^2$ to the the ``haploid'' variance in offspring number.
...
\response{
    ...
    that the probability of fixation is $2s (N_e/N)$; and $N/N_e = \xi^2$.
    We've also added a note to this effect in the text.
\jri{JRI to read text and add/modify to clarify. JRI to add text about long distance.}
 }
...
\response{
 Actually, we ran STRUCTURE more than 3 times for preliminary surveys, and confirmed that our final result is robust.  We keep the text unedited, but thanks!!  
 }
...
To assess the effect of false positives, we have already tried to use more or less stringent cut-offs; we used 0.05, 0.01 and 0.001 as cut-offs of $F_{ST}$ \textit{P} values.   
We obtained qualitatively similar results in any cases, which have been described \st{in the left column in the page 8}.  
}
...
\response{
We agreed this comment.  We analyzed the metabolic networks in maize, which are obtained from MaizeCyc database.  The result was described \st{in the left column in the page 9} and Table S6.
 }
...
\response{
We clarified the part of PHS test in Table S3, File S1, and the text \st{in the right column in the page 5}.  
...
\plr{I don't think they disagree with the word ``quantile''; the complaint is that it's not clear how those quantiles are used? We just say ``to identify outlying $PHS$ values'', but we don't say what quantile was deemed ``outlying''.}
\comst{OK.  Thank you, Peter!!  I tried to make it clearer in the main text and Supporting file S1. (not sure it's ok, though...)}
}
...
We used the published strength of the domestication bottleneck, but we added the sentence that our $F_{ST}$ outlier approach is fairly independent of the strength of the domestication bottleneck \st{in the right column in the page 10}.
}
...
\response{ 
We explained the pattern of allele frequency changes in the text in File S1 rather than in the legend.  We tried to make it clearer.  Hope it is understandable.   
}
